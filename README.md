...
- `POST /hosted-checkout/v1/sessions/{sessionId}/cancel`：取消買家已放棄的 session（更換付款方式時）
- `POST /hosted-checkout/v1/sessions/{sessionId}/refunds`：從 WooCommerce 後台發起信用卡退款

`Authorization` header 應使用 `Bearer <Secret Key>`。

建立 session 時不需要額外注入 `merchantId`；外掛會送出 `successUrl`、`failureUrl` 與 `cancelUrl`（返回購物車），再把回傳的 `id` 視為必要欄位。若 OEN API 未回傳非空 session id，結帳流程會直接失敗，避免將空的 `_oen_session_id` 寫入訂單後關閉 stale-attempt 保護。若同一張未付款訂單再次觸發 `process_payment()`，外掛會先在每張訂單的 advisory lock 內，用既有 `_oen_session_id` 呼叫 `GET /hosted-checkout/v1/sessions/{sessionId}`；只要該 session 仍在進行中、且訂單目前的付款方式與建立該 session 時相同，就會直接重導回已儲存的 `_oen_checkout_url`。若買家回上一頁改選了其他付款方式，外掛會為新方式建立新的 session 並寫入訂單，之後才以 `POST /hosted-checkout/v1/sessions/{sessionId}/cancel` 取消被放棄的舊 session；取消屬 best-effort，失敗時只在訂單留下備註、不阻擋導向。若既有 session 驗證失敗，流程會 fail-closed 並向顧客顯示錯誤，不會偷偷建立新的 live session。只有在先前 session 已進入 `completed`、`charged`、`failed`、`expired`、`cancelled` 等不可重用狀態，或付款方式已變更時，才建立新的 attempt。
...
- `POST /hosted-checkout/v1/sessions/{sessionId}/cancel` to cancel a session the buyer has walked away from (after a payment method change)
- `POST /hosted-checkout/v1/sessions/{sessionId}/refunds` to refund a card payment from the WooCommerce order screen

Send `Authorization: Bearer <Secret Key>` when calling these endpoints.

The plugin does not inject `merchantId` for the v1 secret-key contract. It sends `successUrl`, `failureUrl`, and `cancelUrl` (back to cart), and treats the returned session `id` as required. If the Hosted Checkout create response omits or empties that field, checkout fails instead of saving an empty `_oen_session_id` and weakening stale-attempt protection. When `process_payment()` runs again for the same unpaid order, the plugin first checks the stored `_oen_session_id` inside an order-level advisory lock with `GET /hosted-checkout/v1/sessions/{sessionId}` and reuses the saved `_oen_checkout_url` while that session remains in flight and the order's current payment method is the one the session was created for. If the buyer went back and picked a different payment method, the plugin creates a new session for that method, stores it on the order, and only then cancels the abandoned session with `POST /hosted-checkout/v1/sessions/{sessionId}/cancel`; cancellation is best-effort and a failure only leaves an order note, it never blocks the redirect. If verification of that stored session fails, checkout fails closed instead of silently creating a new live session. A new attempt is created only after the earlier session is already in a terminal, non-reusable state such as `completed`, `charged`, `failed`, `expired`, or `cancelled`, or when the payment method has changed.
...
`refund.succeeded` events are mirrored into WooCommerce refunds: the order is found by matching `data.sessionId` against the stored `_oen_session_id`, a refund is created for `data.amount`, and the OEN refund id is recorded so the paired `refund.created`/`refund.succeeded` events and any redelivery never refund twice (`refund.created` is acknowledged without acting). If the event arrives while the merchant is refunding the same order from the WooCommerce order screen (the plugin marks the order in-progress before the refund request goes out, for at most 300 seconds), the handler creates no refund and records no refund id; it answers `409` so OEN redelivers the event later. By then the admin path has either succeeded and recorded the refund id (the redelivery is a duplicate) or failed and released the mark (the redelivery mirrors the refund), and either way the order ends up with exactly one WooCommerce refund. Because refund events have no server-side re-verification fallback, their authenticity rests entirely on the signature, so **refund events are rejected outright when no Webhook Secret is configured**.
