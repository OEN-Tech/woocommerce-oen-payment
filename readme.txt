...
* The plugin registers its webhook automatically on first save and stores the returned id and signing secret, so the Webhook Secret no longer has to be copied by hand. It reconciles by URL — an existing webhook for this site (e.g. a manually created one) is adopted and its secret rotated in place rather than duplicated, and a manually set secret is never overwritten on an ordinary save. Tick "Re-register webhook" to refresh the URL, events, and signing secret after changing your site URL, MerchantID, or environment (changing the site URL leaves the old webhook in OEN to be removed manually)
